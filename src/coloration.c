...
static CouleursGraphe * cgPrecedent(const CouleursGraphe * cg, const Sommet * s)
{
    ...
    else if (egalSom(cgSommetTete(cgSuivant(cg)), s)) return (CouleursGraphe *)cg;
...
static CouleursGraphe * cgInitAux(CouleursGraphe * cg, const Graphe * g)
{
...
{
    if (cgEstVide(cg)) return NULL;
    else if (egalSom(cgSommetTete(cg), s)) return (CouleursGraphe *)cg;
